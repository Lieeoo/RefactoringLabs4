...
function Page() {
	...
	return (
		<div>
			< TopPanel />
			<div className="Test">
				<div id="ClList" className="ClCl">
				</div>
				<div id="aaa" className="workspaceClassWork">
					<div className="ListSt">
					<a href="NewSt"><button className="profile-button"><img src="https://i.ibb.co/4mZtQCb/user-student-add.png" className="profile-button2"></img></button></a>
						<p>Состав класса</p>
						<div id="Students">
						</div>							
					</div>
					<div id="SetSP" className="SetSPClass">
					</div>
				</div>
			</div>
			<div id= "a"></div>
...
async function enter(a, result) {
	...
	if ((localStorage.thisSt==a.id)&&(document.getElementById('SetSP').style.display == "block")){
		document.getElementById('SetSP').style.display = "none";
		let j = 0;
		var thisid;
		while (j<result.count){
			thisid=result.rows[j].id+"st";
	...
	let i=0;
	while(result.rows[i].id!=newid){
		i++;
	}
	document.getElementById("fio").innerHTML = result.rows[i].fullname;
	if (result.rows[i].sex==2) {document.getElementById("sex").innerHTML = "мужской";}
	else {document.getElementById("sex").innerHTML = "женский";}
	document.getElementById("PFDO").innerHTML = result.rows[i].PFDO;
	let family_id;
	family_id = result.rows[i].family_id;
	...
	document.getElementById('profileEdit').onclick = function() {classtoomProfileStudentEdit(result.rows[i].id, result.rows[i].name, result.rows[i].fullname, result.rows[i].sex,  result.rows[i].birthday, result.rows[i].group_of_risk, result.rows[i].PFDO, result.rows[i].family_id, resultedit.family_status, resultedit.material_condition, resultedit.educationMother, resultedit.motherStat, resultedit.educationFather, resultedit.fatherStat, result.rows[i].class_ID, result, a);};
	document.getElementById("profileDelete").onclick = function() {enter12(newid, result.rows[i].class_ID);};
	if(document.getElementById('SetSP').style.display == "none") {
		document.getElementById('SetSP').style.display = "block";
	}
	localStorage.thisSt = newid+"st";
	let j = 0;
	var thisid;
	while (j<result.count){
		thisid=result.rows[j].id+"st";
...
async function enter4(a) {
	...
	while(i<result.count){
		let result2 = JSON.stringify(i+1 + ". " + result.rows[i].fullname);
		var result3 = result2.substring(1, result2.length-1);
		let p = document.createElement('div')
		p.className = 'TextStyleN';
		p.id=result.rows[i].id+"st";
